...
int solve(std::vector<std::unordered_set<int>> &graph, int r, int b, int movenr, int n){
	if (r == n){
		return 0;
	}
	if (b == n){
	...
	if (movenr % 4 == 0){
		...
		for (std::unordered_set<int>::iterator it = graph.at(r).begin(); it != graph.at(r).end(); it++){
			if (solve(graph, *it, b, movenr+1, n) == 0){
				strategyHolmes = 0;
				break;
			}
		}
		...
		for (std::unordered_set<int>::iterator it = graph.at(b).begin(); it != graph.at(b).end(); it++){
			if (solve(graph, r, *it, movenr+1, n) == 1){
				strategyMoraty = 1;
				break;
			}
		}
		...
		for (std::unordered_set<int>::iterator it = graph.at(b).begin(); it != graph.at(b).end(); it++){
			if (solve(graph, r, *it, movenr+1, n) == 0){
				strategyMoraty = 0;
				break;
			}
		}
		...
		for (std::unordered_set<int>::iterator it = graph.at(r).begin(); it != graph.at(r).end(); it++){
			if (solve(graph, *it, b, movenr+1, n) == 1){
				strategyHolmes = 1;
				break;
			}
		}
...
void testcase(){
	...
	int res = solve(graph, r-1, b-1, 0, n-1);
